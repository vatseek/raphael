...
/***/ function(module, exports, __webpack_require__) {
	...
	var _raphael = __webpack_require__(160);
	...
	var paper = (0, _raphael2.default)(10, 50, 320, 200);
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function (root, factory) {
		if (true) {
			...
			!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(160)], __WEBPACK_AMD_DEFINE_RESULT__ = function (Raphael) {
	   ...
	   */
			ft.updateHandles = function () {
				...
				ft.axes.map(function (axis) {
					if (ft.handles[axis]) {
						cx = ft.attrs.center.x + ft.attrs.translate.x + radius[axis] * ft.opts.distance * Math.cos(rad[axis]), cy = ft.attrs.center.y + ft.attrs.translate.y + radius[axis] * ft.opts.distance * Math.sin(rad[axis]), viewBoxRatio = { x: 1, y: 1 };
				...
				if (ft.bbox) {
					ft.bbox.toFront().attr({
						path: [['M', corners[0].x, corners[0].y], ['L', corners[1].x, corners[1].y], ['L', corners[2].x, corners[2].y], ['L', corners[3].x, corners[3].y], ['L', corners[0].x, corners[0].y]]
					});
				...
				if (ft.opts.rotate.indexOf('self') >= 0) {
					radius = Math.max(Math.sqrt(Math.pow(corners[1].x - corners[0].x, 2) + Math.pow(corners[1].y - corners[0].y, 2)), Math.sqrt(Math.pow(corners[2].x - corners[1].x, 2) + Math.pow(corners[2].y - corners[1].y, 2))) / 2;
				}
	   ...
	   */
			ft.showHandles = function () {
				...
				ft.axes.map(function (axis) {
					...
					ft.handles[axis].line = paper.path(['M', ft.attrs.center.x, ft.attrs.center.y]).attr({
						stroke: ft.opts.attrs.stroke,
						'stroke-dasharray': '- ',
						opacity: .5
					});
	
					ft.handles[axis].disc = paper.circle(ft.attrs.center.x, ft.attrs.center.y, ft.opts.size.axes).attr(ft.opts.attrs);
				...
				if (ft.opts.draw.indexOf('bbox') >= 0) {
					...
					for (i = ft.opts.scale.indexOf('bboxCorners') || ft.opts.customCorners >= 0 ? 0 : 4; i < (ft.opts.scale.indexOf('bboxSides') === -1 ? 4 : 8); i++) {
						...
						if (handle.isCorner && ft.opts.customCorners) {
							handle.element = paper.image(ft.opts.customCorners.corners[i].image, ft.attrs.center.x, ft.attrs.center.y, ft.opts.customCorners.size, ft.opts.customCorners.size);
						} else {
							handle.element = paper.rect(ft.attrs.center.x, ft.attrs.center.y, ft.opts.size[handle.isCorner ? 'bboxCorners' : 'bboxSides'] * 2, ft.opts.size[handle.isCorner ? 'bboxCorners' : 'bboxSides'] * 2).attr(ft.opts.attrs);
				...
				if (ft.opts.drag.indexOf('center') !== -1) {
					...
					ft.handles.center.disc = paper.circle(ft.attrs.center.x, ft.attrs.center.y, ft.opts.size.center).attr(ft.opts.attrs);
				...
				handles.map(function (handle) {
					handle.element.drag(function (dx, dy) {
						...
						cx = dx + handle.element.ox, cy = dy + handle.element.oy;
						...
						opposite = cx - ft.o.center.x - ft.o.translate.x - (ft.opts.customCorners ? ft.opts.customCorners.distance : 0), adjacent = cy - ft.o.center.y - ft.o.translate.y - (ft.opts.customCorners ? ft.opts.customCorners.distance : 0), hypotenuse = Math.sqrt(Math.pow(opposite, 2) + Math.pow(adjacent, 2));
						...
						if (handle.isCorner) {
							hypotenuse = Math.sqrt(Math.pow(ft.o.size.x * ft.o.scale.x, 2) + Math.pow(ft.o.size.y * ft.o.scale.y, 2)) / 2, opposite = ft.o.size.x * ft.o.scale.x / 2, pos = {
					...
					ft.handles.bbox.map(function (handle) {
						handle.element.drag(function (dx, dy) {
							...
							if (handle.isCorner && ft.opts.keepRatio.indexOf('bboxCorners') !== -1) {
								ratio = ft.attrs.size.x * ft.attrs.scale.x / (ft.attrs.size.y * ft.attrs.scale.y), tdy = rx * handle.x * (1 / ratio), tdx = ry * handle.y * ratio;
							...
							var angle = (360 - ft.attrs.rotate) % 360 / 180 * Math.PI,
				...
				rotate = ft.opts.rotate.indexOf('self') >= 0, scale = ft.opts.scale.indexOf('self') >= 0;
			...
			ft.apply = function () {
				ft.items.map(function (item, i) {
					...
					if (ft.opts.animate) {
						...
						item.el.animate({ transform: ['R', rotate, center.x, center.y, 'S', scale.x, scale.y, center.x, center.y, 'T', translate.x, translate.y] + ft.items[i].transformString }, ft.opts.animate.delay, ft.opts.animate.easing, function () {
							asyncCallback(['animate end']);
	
							ft.updateHandles();
						});
					} else {
						item.el.transform(['R', rotate, center.x, center.y, 'S', scale.x, scale.y, center.x, center.y, 'T', translate.x, translate.y] + ft.items[i].transformString);
	   ...
	   */
			...
			ft.items.map(function (item, i) {
				if (item.el._ && item.el._.transform && _typeof(item.el._.transform) === 'object') {
	   ...
	   */
			function applyLimits(bbox) {
				...
				if (bbox && ft.opts.snap.drag) {
					x = bbox.x, y = bbox.y, [0, 1].map(function () {
	   ...
	   */
			function cloneObj(obj) {
				...
				for (i in obj) {
					clone[i] = _typeof(obj[i]) === 'object' ? cloneObj(obj[i]) : obj[i];
				}
